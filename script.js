...
const selectElements = (selectors = []) => {
...
const waitMsg = [
  ...
  'wait a second. i will answer that',
...
recognition.onerror = (event) => {
  ...
  stopBtn.click()
  answer.value = text
  interimTxt.innerHTML =
    "<span style='color:red'>Need Internet Connection</span> "
    ...
    if (answer.value.length > 0) {
      ...
      setTimeout(() => {
        if (!window.speechSynthesis.speaking && !answered) {
          tellMe(
            waitMsg[Math.floor(Math.random() * waitMsg.length)],
            voiceIndex,
            pitch,
            rate,
            true,
...
const compressMsg = async () => {
  ...
  try {
    workerURL =
      workerurlTxt.value || 'https://speakmate.tepax23408.workers.dev/'
...
document.querySelector('#install').addEventListener('click', (event) => {
  if (bipEvent) {
    ...
    interimTxt.innerHTML =
      "<span style='color:red;'>To install the app look for 'Add to Homescreen' or 'Install' option in your browser's menu</span>"
    setTimeout(() => (interimTxt.innerHTML = ''), 3000)
  }
})
